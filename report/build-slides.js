...
// 3 ─ Objectives & scope
{
  ...
  bulletsText(s, ["ฟังก์ชันหลัก 3 ฟังก์ชัน + แดชบอร์ดส่งออก Excel/PDF", "ข้อมูลจริง 3 วิทยาเขต 87 อาคาร 457 ห้อง", "เข้าสู่ระบบแบบจำลอง CMU Account และสิทธิ์ตามบทบาท"], 7.1, 2.4, 5.4, 2.3, 15);
...
// 6 ─ NFR
{
  ...
  const groups = [
    ["ความปลอดภัย", ["เซสชันทุกหน้า หมดเวลา 30 นาที", "ตรวจสิทธิ์ฝั่งเซิร์ฟเวอร์", "bcrypt + HTTPS", "เบอร์โทรเห็นเฉพาะช่างที่รับงาน", "PDPA ก่อนใช้งาน", "DB role แยก smartcmu_app"]],
...
{
  const s = page("แผนภาพกิจกรรม", "แจ้งซ่อม และ งานซ่อมของช่าง");
...
// 11 ─ Activity admin + confirm + state
{
  const s = page("แผนภาพกิจกรรมและสถานะ", "คัดกรอง → มอบหมาย → ยืนยันผล");
  img(s, DIA("activity-admin"), 0.4, 1.5, 3.4, 5.85);
  img(s, DIA("activity-confirm"), 3.9, 1.6, 5.0, 2.9);
  img(s, DIA("state"), 9.1, 1.5, 3.8, 5.85);
  s.addText("10 สถานะ ควบคุมที่ฟังก์ชันเดียวฝั่งเซิร์ฟเวอร์\nยืนยัน / ครบ 3 วัน → ปิดงาน\nยังไม่หาย → กลับไปรับเรื่องแล้ว", { x: 3.9, y: 4.8, w: 5.0, h: 1.8, fontFace: FONT, fontSize: 15, color: C.ink, valign: "top" });
}
