...
export async function getDashboard(range: DashboardRange, opts: { includeRows?: boolean } = {}) {
  ...
  const [[kpi], [closeTime], [rating], daily, byStatus, buildings, categories, technicians, oldestUrgent, rows] = await Promise.all([
    ...
    opts.includeRows
      ? sql<{ code: string; created_at: Date; status: Status; urgency: Urgency; category: string; campus: string; building: string; floor: number; room: string; reporter: string; technician: string | null; closed_at: Date | null; score: number | null }[]>`
